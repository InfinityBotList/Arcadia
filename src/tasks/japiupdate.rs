...
pub async fn japi_updater(ctx: &serenity::all::Context) -> Result<(), crate::Error> {
    ...
    let bots_to_update = sqlx::query!(
        "SELECT bot_id FROM bots WHERE (type = 'approved' OR type = 'certified') AND NOW() - last_stats_post > INTERVAL '3 days' AND (last_japi_update IS NULL OR NOW() - last_japi_update > INTERVAL '3 days') ORDER BY RANDOM() LIMIT 2"
    )
    .fetch_all(pool)
    .await?;
